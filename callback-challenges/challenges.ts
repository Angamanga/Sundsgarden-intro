...
const addTwo = (inputNumber: number): number => {
...
const addS = (inputString: string): string => {
...
const map = (numberList: number[], callback: Callback): number[] => {
...
const forEach = <T>(inputArray: T[], callback: (input: T) => void): void => {
...
const mapWith = (numberList: number[], callback: Callback): number[] => {
    const newArray: number[] = [];
    forEach(numberList, (number: number) => {
        newArray.push(callback(number));
    });
    return newArray;
};
